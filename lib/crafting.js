...
export const EPIC_PITY_THRESHOLD = 20;
...
function bumpItemRarity(item, targetRarity) {
  const oldRarity = getRarityById(item.rarity);
  const newRarity = typeof targetRarity === "string" ? getRarityById(targetRarity) : targetRarity;
  const newStats = {};
  for (const [stat, value] of Object.entries(item.stats)) {
    const baseValue = Math.round(value / oldRarity.multiplier);
    newStats[stat] = Math.round(baseValue * newRarity.multiplier);
  }
  const maxDur = getMaxDurability(item.tier, newRarity.id);
  return {
    ...item,
    rarity: newRarity.id,
    stats: newStats,
...
export function applyCritCraft(item, prestige) {
  ...
  return { ...bumpItemRarity(item, nextRarity), critCraft: true };
}

export function applyEpicPity(item) {
  if (item.rarity === "epic") return item;
  return { ...bumpItemRarity(item, "epic"), epicPity: true };
}
