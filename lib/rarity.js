...
// Returns the next rarity tier, or null if already at max (epic)
export function getNextRarity(rarityId) {
  const idx = RARITIES.findIndex((r) => r.id === rarityId);
...
export function getSellValue(recipe, rarity, level) {
  const r = typeof rarity === "string" ? getRarityById(rarity) : rarity;
  if (!r) return 1; // consumables and items without rarity sell for 1 gold
