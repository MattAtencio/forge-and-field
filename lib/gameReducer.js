...
export function gameReducer(state, action) {
  switch (action.type) {
    ...
    case "TICK": {
      ...
      // Check crafting queue completions
      const completedCrafts = [];
      const stillCrafting = [];
      for (const craft of state.craftingQueue) {
        if (now >= craft.startedAt + craft.duration) {
